...
export const bugs = pgTable("bugs", {
  ...
  severity: text("severity", { enum: ["Critical", "Major", "Minor", "Trivial"] }).notNull(),
...
// Automation Script Runs model
export const automationRuns = pgTable("automation_runs", {
  id: serial("id").primaryKey(),
  scriptId: integer("script_id").notNull(),
  status: text("status", { enum: ["Passed", "Failed", "Running", "Aborted", "Pending"] }).notNull(),
  ...
  logs: text("logs"),
  screenshotUrls: json("screenshot_urls").default([]),
  executedById: integer("executed_by_id").notNull(),
...
export const automationSchedules = pgTable("automation_schedules", {
  ...
  createBugOnFailure: boolean("create_bug_on_failure").default(false),
  createdById: integer("created_by_id").notNull(),
  updatedById: integer("updated_by_id"),
  createdAt: timestamp("created_at").defaultNow(),
  updatedAt: timestamp("updated_at"),
...
// Automation Environment model
export const automationEnvironments = pgTable("automation_environments", {
  id: serial("id").primaryKey(),
  name: text("name").notNull(),
  url: text("url").notNull(),
  description: text("description"),
  type: text("type", { enum: ["Development", "Testing", "Staging", "Production"] }).notNull(),
  variables: json("variables").default({}),
  status: text("status", { enum: ["Online", "Offline", "Maintenance"] }).notNull().default("Online"),
  createdById: integer("created_by_id").notNull(),
  createdAt: timestamp("created_at").defaultNow(),
  updatedAt: timestamp("updated_at"),
});

export const insertAutomationEnvironmentSchema = createInsertSchema(automationEnvironments).omit({
  id: true,
  createdAt: true,
  updatedAt: true,
});

// Types for Automation entities
...
export type AutomationEnvironment = typeof automationEnvironments.$inferSelect;
export type InsertAutomationEnvironment = z.infer<typeof insertAutomationEnvironmentSchema>;
