...
export const spreadsheets = pgTable("spreadsheets", {
  ...
  name: text("name").notNull(),
  projectId: integer("project_id").notNull(),
  data: json("data").$type<any[][]>().notNull().default([]),
  columns: json("columns").$type<string[]>().notNull().default([]),
  createdById: integer("created_by_id").notNull(),
  createdAt: timestamp("created_at").defaultNow(),
  updatedAt: timestamp("updated_at"),
});
...
// Cell value schema for matrix operations
export const cellValueSchema = z.object({
  rowModuleId: z.number(),
  colModuleId: z.number(),
  projectId: z.number(),
  value: z.string(),
...
export const cellValueSchemaUI = z.object({
