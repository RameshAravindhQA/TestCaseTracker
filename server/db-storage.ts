...
export class DatabaseStorage implements IStorage {
  ...
  async getUserConversations(userId: number): Promise<any[]> {
    ...
    return userConversations.map(conv => {
      const conversationMessages = this.messages.filter(m => m.conversationId === conv.id);
      const lastMessage = conversationMessages.sort((a, b) => new Date(b.createdAt).getTime() - new Date(a.createdAt).getTime())[0] || null;
      const unreadCount = conversationMessages.filter(m => m.senderId !== userId && (!m.readBy || !m.readBy.includes(userId))).length;
  ...
  async getMessagesByChat(chatId: number): Promise<any[]> {
    const chatMessages = this.messages.filter(m => m.conversationId === chatId);
    console.log(`Storage: Retrieved ${chatMessages.length} messages for chat ${chatId}`);
  ...
  async getDirectConversation(userId1: number, userId2: number): Promise<any | null> {
    ...
    if (existing) {
      console.log(`Storage: Found existing direct conversation ${existing.id} between users ${userId1} and ${userId2}`);
    }
