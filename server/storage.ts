...
export interface IStorage {
  ...
  async getProjectUsers(projectId: number): Promise<Array<{ id: number; name: string; email: string }>>;

  // Automation Script Management
  createAutomationScript(script: any): any;
  getAutomationScript(id: number): any | undefined;
  getAutomationScripts(projectId?: number): any[];
  updateAutomationScript(id: number, updates: any): any | undefined;
  deleteAutomationScript(id: number): boolean;
  addScriptExecution(scriptId: number, execution: any): void;
  getScriptExecutions(scriptId: number): any[];

   // Todo List Methods
   getTodoLists(userId: number) :Promise<any[]>;
   getTodoList(id: number) :Promise<any | null>;
   createTodoList(data: any) :Promise<any>;
   updateTodoList(id: number, userId: number, data: any) :Promise<any | null>;
   deleteTodoList(id: number, userId: number) :Promise<boolean>;

  // Todo Methods
   getTodosByUserId(userId: number) :Promise<any[]>;
   getTodosByListId(listId: number) :Promise<any[]>;
   getTodo(id: number) :Promise<any | null>;
   createTodo(data: any) :Promise<any>;
   updateTodo(id: number, userId: number, data: any) :Promise<any | null>;
...
class MemStorage implements IStorage {
  ...
  private conversationMessages = new Map<number, any[]>(); // conversationId -> messages array
  ...
  async getBugs(userId?: string): Promise<Bug[]> {
    ...
    if (userId) {
      ...
      return bugs.filter(bug => userProjectIds.includes(bug.projectId));
    }
    return bugs;
  }
  ...
  async getCustomerById(id: number): Promise<Customer | null>{    return this.customers< Map<number, any>();
  }
...
return Array.from(this.matrixCells.values());
  ...
  async createChatMessage(messageData: any): Promise<any> {
    ...
    console.log(`Storage: Created chat message ${id} for conversation ${messageData.conversationId}`);
    return message;
  }

  async getMessagesByChat(chatId: number): Promise<any[]> {
    // Check cache first for better performance
    if (this.conversationMessages.has(chatId)) {
    ...
    // Get all messages for this conversation from main storage
  ...
  async getDirectConversation(userId1: number, userId2: number): Promise<any | null> {
    try {
      // Check existing conversations from the Map storage
      for (const [convId, conversation] of this.conversations.entries()) {
        if (conversation.type === 'direct' && 
          ...
          console.log(`Storage: Found existing direct conversation ${convId} between users ${userId1} and ${userId2}`);
          return conversation;
        }
      }

      console.log(`Storage: No existing conversation found between users ${userId1} and ${userId2}`);
      return null;
    } catch (error) {
  ...
  async getChatMessages(conversationId: number, limit?: number, offset?: number): Promise<any[]> {
    ...
    if (this.conversationMessages.has(conversationId)) {
      const cachedMessages = this.conversationMessages.get(conversationId)!;
      if (cachedMessages.length > 0) {
        console.log(`Storage: Retrieved ${cachedMessages.length} cached messages for conversation ${conversationId}`);
        return cachedMessages.sort((a, b) => new Date(a.timestamp).getTime() - new Date(b.timestamp).getTime());
      }
    }
    ...
    const allMessages = Array.from(this.chatMessages.values());
    const messages = allMessages.filter(msg => 
      msg.conversationId === conversationId || msg.chatId === conversationId
    ).sort((a, b) => new Date(a.timestamp).getTime() - new Date(b.timestamp).getTime());

    // Update the cache with fresh data
    this.conversationMessages.set(conversationId, messages);

    console.log(`Storage: Retrieved ${messages.length} messages for conversation ${conversationId}`);
    return messages;
  }

    getGitHubConfigs(): any[] {
    return this.githubConfigs;
  }

  deleteGitHubConfig(id: number): boolean {
    const index = this.githubConfigs.findIndex(config => config.id === id);
  ...
  // Automation Script Management
  private automationScripts = new Map<number, any>();
  private scriptExecutions = new Map<number, any[]>();
  ...
  createAutomationScript(script: any): any {
    ...
    this.automationScripts.set(id, script);
    return script;
  }

  getAutomationScript(id: number): any | undefined {
    return this.automationScripts.get(id);
  }
  ...
  updateAutomationScript(id: number, updates: any): any | undefined {
    const script = this.automationScripts.get(id);
    if (script) {
      const updatedScript = { ...script, ...updates, updatedAt: new Date().toISOString() };
      this.automationScripts.set(id, updatedScript);
  ...
  deleteAutomationScript(id: number): boolean {
    return this.automationScripts.delete(id);
  }

  addScriptExecution(scriptId: number, execution: any): void {
      const id = this.getNextId();
      execution.id = id;
      execution.createdAt = new Date().toISOString();
      execution.updatedAt = new Date().toISOString();

    if (!this.scriptExecutions.has(scriptId)) {
      this.scriptExecutions.set(scriptId, []);
    }
    this.scriptExecutions.get(scriptId)?.push(execution);
  }

  getScriptExecutions(scriptId: number): any[] {
    return this.scriptExecutions.get(scriptId) || [];
  }

   // Todo List Methods
  async getTodoLists(userId: number): Promise<any[]> {
    return Array.from(this.todoLists.values())
      .filter(todoList => todoList.userId === userId)
      .sort((a, b) => new Date(b.createdAt).getTime() - new Date(a.createdAt).getTime());
  }

  async getTodoList(id: number): Promise<any | null> {
  ...
  async updateTodoList(id: number, userId: number, data: any): Promise<any | null> {
    const todoList = this.todoLists.get(id);
     if (!todoList) {
      return null;
    }
  ...
  async getTodosByListId(listId: number): Promise<any[]> {
     return Array.from(this.todos.values())
      .filter(todo => todo.todoListId === listId)
      .sort((a, b) => new Date(b.createdAt).getTime() - new Date(a.createdAt).getTime());
  }

  async getTodo(id: number): Promise<any | null> {
  ...
  async updateTodo(id: number, userId: number, data: any): Promise<any | null> {
    const todo = this.todos.get(id);
    if (!todo) {
      return null;
    }
