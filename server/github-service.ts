...
export class GitHubService {
  ...
  constructor() {
    console.log('GitHub Service initialized with repository:', this.defaultConfig);
  }
  ...
  async testConnection(config?: any): Promise<any> {
    const testConfig = config || this.defaultConfig;
    const { repoOwner, repoName, accessToken } = testConfig;

    try {
      const response = await fetch(`${this.baseUrl}/repos/${repoOwner}/${repoName}`, {
        method: 'GET',
        headers: {
