...
import { Send, MessageCircle, Users, Settings, Plus, Search, UserPlus, Hash, Lock, Reply, Edit3, Trash2, Pin, PinOff, Phone, Video, Paperclip, At, X, Download, Palette, Mic, MicOff, Play, Pause, Square } from 'lucide-react';
...
export function Messenger() {
  ...
  const startJitsiCall = (isVideo: boolean = true) => {
    ...
    setJitsiMeetUrl(`https://${domain}/${roomName}`);
      ...
      <div className="flex-1 flex flex-col">
        {selectedChat ? (
          <>
            ...
            {showPinnedBoard && pinnedMessages.length > 0 && (
              <div className="bg-yellow-50 border-t border-yellow-200 p-4 max-h-40 overflow-y-auto">
                <div className="flex justify-between items-center mb-2">
                  <h4 className="text-sm font-medium text-yellow-800">Pinned Messages</h4>
                  <Button size="sm" variant="ghost" onClick={() => setShowPinnedBoard(false)}>
                    <X className="h-3 w-3" />
                  </Button>
                </div>
                <div className="space-y-2">
                  {pinnedMessages.map((message) => (
                    <div key={message.id} className="p-2 bg-white rounded border">
                      <div className="text-xs font-medium text-gray-600">{message.userName}</div>
            ...
            <Dialog open={showJitsiDialog} onOpenChange={setShowJitsiDialog}>
              <DialogContent className="max-w-4xl max-h-[80vh]">
                ...
                <div className="flex justify-between items-center">
                  <p className="text-sm text-gray-600">Room: {jitsiMeetUrl}</p>
                  <Button onClick={() => navigator.clipboard.writeText(jitsiMeetUrl)}>
                    Copy Link
                  </Button>
                </div>
              </DialogContent>
