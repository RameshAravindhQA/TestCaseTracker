...
export class DatabaseStorage implements IStorage {
  ...
  async getDirectConversation(userId1: number, userId2: number): Promise<any> {
    try {
  ...
  async createScreenRecording(data: any): Promise<any> {
    try {
      // Create demo screen recording record
      return {
        id: Date.now(),
        ...data,
        createdAt: new Date().toISOString()
  ...
  async createOnlyOfficeDocument(data: any): Promise<any> {
    try {
      // Create demo OnlyOffice document
      return {
      ...
      console.error("Error creating OnlyOffice document:", error);
      throw error;
    }
  }

  async getOnlyOfficeDocument(documentId: string): Promise<any> {
    try {
      ...
      return {
        ...
        fileType: "docx",
        key: `doc_${Date.now()}`,
        url: `/api/onlyoffice/documents/content/${documentId}`,
        createdAt: new Date().toISOString()
      };
    } catch (error) {
      console.error("Error getting OnlyOffice document:", error);
      return null;
    }
  }

  async updateOnlyOfficeDocument(documentId: string, data: any): Promise<any> {
    try {
      ...
      console.error("Error updating OnlyOffice document:", error);
      throw error;
    }
  }
