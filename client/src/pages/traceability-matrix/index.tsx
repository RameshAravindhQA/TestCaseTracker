...
export default function TraceabilityMatrixPage() {
  ...
  // Fetch matrix data for selected project
  const { data: projectMatrixData } = useQuery({
    queryKey: ["/api/projects", selectedProjectId, "matrix/data"],
    queryFn: async () => {
      if (!selectedProjectId) return {};
      const response = await apiRequest("GET", `/api/projects/${selectedProjectId}/matrix/data`);
      if (!response.ok) return {};
      return response.json();
    },
    enabled: !!selectedProjectId,
  ...
  useEffect(() => {
    ...
    console.log('Project matrix data:', projectMatrixData);
    ...
    if (selectedProjectId) {
      ...
      // First, load from individual matrix cells
      (projectCells || []).forEach((cell: MatrixCell) => {
        const key = `${cell.rowModuleId}-${cell.colModuleId}`;
        cellsObj[key] = cell;
      });

      // Then, load from saved matrix data (this takes precedence)
      if (projectMatrixData && Object.keys(projectMatrixData).length > 0) {
        Object.entries(projectMatrixData).forEach(([key, markerId]) => {
          const [rowModuleId, colModuleId] = key.split('-').map(Number);
          
          // Update existing cell or create new one
          cellsObj[key] = {
            id: cellsObj[key]?.id || `temp-${key}`,
            rowModuleId,
            colModuleId,
            projectId: selectedProjectId,
            value: markerId as string,
            createdById: cellsObj[key]?.createdById || 1,
            createdAt: cellsObj[key]?.createdAt || new Date().toISOString(),
            updatedAt: new Date().toISOString()
          };
        });
      }

      console.log('Setting matrix cells:', Object.keys(cellsObj).length, 'cells');
  ...
  }, [selectedProjectId, projectModules, projectMarkers, projectCells, projectMatrixData]);
  ...
  const saveMatrixMutation = useMutation({
    mutationFn: async () => {
      const matrixData = Object.entries(matrixCells).map(([key, cell]) => {
        const [rowModuleId, colModuleId] = key.split('-').map(Number);
        return {
          rowModuleId,
          colModuleId,
      ...
      const response = await apiRequest("POST", `/api/projects/${selectedProjectId}/matrix/save`, {
        projectId: selectedProjectId,
    ...
    onSuccess: () => {
      ...
      queryClient.invalidateQueries({ queryKey: ["/api/projects", selectedProjectId, "matrix/cells"] });
      queryClient.invalidateQueries({ queryKey: ["/api/projects", selectedProjectId, "matrix/data"] });
    },
    onError: (error: any) => {
      toast({
        title: "Save Failed",
  ...
  const handleRefreshMatrix = () => {
    ...
    // Refresh all queries for the current project
    ...
    toast({
      ...
      description: "Traceability matrix data has been refreshed",
    });
  };
        ...
        <div className="flex items-center justify-between">
          ...
          <div className="flex items-center gap-4">
            ...
            <ProjectSelect
              ...
              onChange={(value) => {
                const projectId = (value === "" || value === "no-projects" || value === "loading") ? null : parseInt(value);
                setSelectedProjectId(projectId);
              }}
              projects={projects || []}
              ...
              <Card>
                ...
                <CardContent>
                  <div className="overflow-x-auto border rounded-lg">
                    <Table>
                      <TableHeader>
                        <TableRow className="bg-gray-50 h-[140px]">
                          <TableHead className="w-[300px] sticky left-0 bg-gray-50 z-10 border-r-2 border-gray-200 font-semibold align-bottom">
                          ...
                          {modules.map(module => (
                            <TableHead key={module.id} className="text-center w-[80px] min-w-[80px] max-w-[80px] bg-gray-50 border-r border-gray-200 p-2 relative">
                              <div className="flex items-center justify-center h-full w-full">
                                <div 
                                  className="font-medium text-xs text-gray-700 flex items-center justify-center"
                                  style={{ 
                                  ...
                                  title={module.name}
                                >
                                  {module.name.length > 12 ? `${module.name.substring(0, 12)}...` : module.name}
                                </div>
                              </div>
                            </TableHead>
                          ))}
                        ...
                        {modules.map((rowModule, rowIndex) => (
                          <TableRow key={rowModule.id} className={rowIndex % 2 === 0 ? 'bg-white' : 'bg-gray-50/30'}>
                            <TableCell className="sticky left-0 bg-inherit z-10 border-r-2 border-gray-200 p-4">
                              <div className="flex items-center gap-3">
                                ...
                                <div className="min-w-0 flex-1">
                                  <div className="font-medium text-sm truncate" title={rowModule.name}>
                                    {rowModule.name}
                                  </div>
                                  {rowModule.description && (
                                    <div className="text-xs text-muted-foreground truncate max-w-[200px] mt-1" title={rowModule.description}>
                                      {rowModule.description}
                                    </div>
                                  )}
                                </div>
                            ...
                            {modules.map(colModule => {
                              ...
                              return (
                                <TableCell key={colModule.id} className="text-center p-2 border-r border-gray-200 w-[80px] min-w-[80px] max-w-[80px]">
                                  <div
                                    className={`flex justify-center items-center h-12 w-full mx-auto rounded-lg border-2 ${
                                      isDisabled 
                                    ...
                                    {isDisabled
                                      ...
                                      : matrixCell?.value
                                        ? (() => {
                                            ...
                                            return (
                                              <div 
                                                className="px-3 py-2 rounded text-xs font-medium text-center break-words w-full"
                                                style={{ 
                                                  backgroundColor,
