  ...
  const navItems: NavItem[] = useMemo(() => [
    ...
    },
    {
      ...
      children: [
        ...
        {
          name: "Enhanced Sheets",
          href: "/enhanced-spreadsheets",
      ...
      <nav className="flex-1 overflow-y-auto">
        <div className="px-2 py-4 space-y-1">
          {useMemo(() => 
                ...
                <div key={item.href}>
                  {hasChildren ? (
                    <button
                      ...
                      className={cn(
                        "w-full flex items-center justify-between px-2 py-2 text-sm font-medium rounded-md group text-left",
                        location.startsWith(item.href) || item.children?.some(child => location.startsWith(child.href))
                          ? "text-white bg-primary"
                          : "text-gray-700 hover:bg-gray-100 dark:text-gray-200 dark:hover:bg-gray-800"
                      )}
                    >
                      <div className="flex items-center">
                        <span className={cn("mr-3", location.startsWith(item.href) || item.children?.some(child => location.startsWith(child.href)) ? "text-white" : "text-gray-500 dark:text-gray-400")}>
                          {item.icon}
                        </span>
                        {item.name}
                      </div>
                      <ChevronDown 
                        className={cn(
                          ...
                          location.startsWith(item.href) || item.children?.some(child => location.startsWith(child.href)) ? "text-white" : "text-gray-400"
                        )}
                      />
                    </button>
                  ) : (
                  ...
                  {/* Render submenu if item has children and is expanded */}
            ...
            }), [navItems, location, expandedItems])}
        </div>
